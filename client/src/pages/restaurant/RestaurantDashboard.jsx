...
const RestaurantDashboard = () => {
  ...
  const [newItem, setNewItem] = useState({
    ...
    category: '',
    image_url: '',
    isVeg: true, // explicitly set to true for vegetarian by default
  ...
  const fetchMenuItems = async () => {
    try {
      ...
      // Normalize the data to use isVeg consistently in frontend
      const normalizedItems = response.data.map(item => ({
        ...item,
        isVeg: item.is_veg // Map is_veg to isVeg for frontend use
      }));
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          const compressedBase64 = canvas.toDataURL('image/jpeg', 0.7); // Compress with 0.7 quality
  ...
  const handleNewItemSubmit = async (e) => {
    ...
    try {
      ...
      const formattedItem = {
        ...
        is_veg: Boolean(newItem.isVeg) // Add is_veg for API compatibility
      };
  ...
  const handleEditClick = (item) => {
    // Use is_veg from the API response and convert to boolean
    setEditingItem({
      ...item,
      isVeg: Boolean(item.is_veg) // Convert to boolean
    });
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (editingItem.image instanceof File) {
        imageUrl = await handleImageUpload(editingItem.image);
      }
      ...
      const formattedItem = {
        ...
        is_veg: editingItem.isVeg === true // Only send is_veg to match the API's expected format
      };
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`w-5 h-5 ${(item.isVeg || item.is_veg) ? 'bg-green-500' : 'bg-red-500'} rounded-full flex items-center justify-center`}>
                          <span className="text-white text-xs">
                            {(item.isVeg || item.is_veg) ? 'V' : 'N'}
                          </span>
                        </span>
                      </div>
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <label className="flex items-center cursor-pointer">
                      <input
                        ...
                        onChange={() => {
                          console.log('Setting to Non-Veg');
                          setEditingItem(prev => ({
                            ...prev,
                            isVeg: false
                          }));
